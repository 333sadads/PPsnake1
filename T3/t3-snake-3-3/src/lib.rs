...
pub fn greedy_snake_step(n: i32, snake: &[i32], snake_num: i32, snake_ot: &[i32], food_num: i32, foods: &[i32], round:i32) -> i32 {
    ...
    for dir in 0..4 {
        ...
        for i in 0..(4*snake_num) as usize {
            mark  = mark + get_mark(lambda_snake, (x, y), snake_bank[i]);
        }
        for i in 0..food_num as usize {
            mark = mark + get_mark(lambda_food, (x, y), food_bank[i]);
        }
...
fn get_mark(lambda: f64, pt1: (i32, i32), pt2: (i32, i32)) -> f64 {
    let dist = get_dist(pt1, pt2);
    100.0 * lambda / (f64::from(dist) * f64::from(dist) + 0.0001)
}
